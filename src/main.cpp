...
SDL_AppResult SDL_AppIterate(void *appState)
{
    ...
    ImGui::SetNextWindowSize(ImVec2(270.0f * app->scale_factor, 217.0f * app->scale_factor), ImGuiCond_FirstUseEver);
    ...
    // Track if selection changes to optionally reset time
    int previousAnimation = selectedAnimation;
    ImGui::RadioButton("Idle", &selectedAnimation, 0);
    ImGui::RadioButton("Walking", &selectedAnimation, 1);
    ImGui::RadioButton("Running", &selectedAnimation, 2);
    ImGui::End();

    // Pick the active animation pointer
    ozz::animation::Animation *activeAnimation = &app->ozzIdleAnimation;
    if (selectedAnimation == 1)
        activeAnimation = &app->ozzWalkAnimation;
    else if (selectedAnimation == 2)
        activeAnimation = &app->ozzRunAnimation;

    // Optional: Reset animation time if the user switches animations
    if (previousAnimation != selectedAnimation)
    {
        app->ozzAnimationTime = 0.0f;
    }
    ...
    // Advance time and loop within the animation duration
    app->ozzAnimationTime += delta_time;
    float duration = activeAnimation->duration();
    if (duration > 0.0f)
    {
        app->ozzAnimationTime = fmod(app->ozzAnimationTime, duration);
        if (app->ozzAnimationTime < 0.0f)
        {
            app->ozzAnimationTime += duration; // Handle negative fmod if any
        }
    }
    else
    {
    ...
    sampling_job.animation = activeAnimation;
    sampling_job.ratio = activeAnimation->duration() > 0.0f ? (app->ozzAnimationTime / activeAnimation->duration()) : 0.0f;
    sampling_job.context = &app->ozzSamplingContext;

    // Allocate local transforms output buffer
    ozz::vector<ozz::math::SoaTransform> locals;
    locals.resize(app->ozzSkeleton.num_soa_joints());
    sampling_job.output = make_span(locals);

    if (!sampling_job.Run())
    {
        SDL_Log("Failed to sample ozz animation at ratio: %f", sampling_job.ratio);
        return SDL_APP_CONTINUE;
    }
    ...
    conv_job.output = make_span(app->ozzModelMatrices);
...
#if defined(__ANDROID__)
    vec3 textTranslation = { -1.5f, -1.5f, 0.0f };
#else
    vec3 textTranslation = { -0.7f, -0.5f, 0.0f };
#endif
    ...
    // glm_scale(textModelMatrix, textScale);
